...
export default function Contact() {
  ...
  return (
    <section id="contact" style={{ padding: "56px 28px", background: "#26215C", borderTop: "0.5px solid #2D2570" }}>
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
                {[
                  ...
                  <div key={f.key}>
                    <label style={{ fontSize: 11, fontWeight: 500, color: "#2D2570", display: "block", marginBottom: 5 }}>{f.label}</label>
                    <input
                      type={f.type}
                      placeholder={f.placeholder}
                      value={form[f.key as keyof typeof form]}
                      onChange={(e) => setForm({ ...form, [f.key]: e.target.value })}
                      style={{ width: "100%", background: "#fff", border: "0.5px solid #8B7ED4", borderRadius: 7, padding: "9px 12px", fontSize: 13, color: "#26215C" }}
                    />
                  </div>
                ))}
              </div>
              ...
              <div style={{ marginBottom: 10 }}>
                <label style={{ fontSize: 11, fontWeight: 500, color: "#2D2570", display: "block", marginBottom: 5 }}>Project type</label>
              ...
              <div style={{ marginBottom: 10 }}>
                <label style={{ fontSize: 11, fontWeight: 500, color: "#2D2570", display: "block", marginBottom: 5 }}>Tell us about your project</label>
                <textarea
                  placeholder="What are you building? Any timeline or budget in mind?"
                  value={form.message}
                  onChange={(e) => setForm({ ...form, message: e.target.value })}
                  style={{ width: "100%", background: "#fff", border: "0.5px solid #8B7ED4", borderRadius: 7, padding: "9px 12px", fontSize: 13, color: "#26215C", resize: "none", height: 80 }}
                />
              </div>
              ...
              <button 
                ...
                style={{ 
                  width: "100%", 
                  background: loading ? "#8B7ED4" : "#7F77DD", 
                  color: "#E8E4FF", 
                  border: "none", 
                  borderRadius: 7, 
                  padding: 11, 
